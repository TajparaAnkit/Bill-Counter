    ...
    <header className="bg-white/90 backdrop-blur-md border-b border-slate-100 sticky top-0 z-40 shadow-xs">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between gap-4 py-3">
          {/* Left: hamburger (mobile) + logo (mobile only — sidebar shows branding on desktop) */}
          <div className="flex items-center gap-3">
            <button
              onClick={onMenuClick}
              className="grid h-10 w-10 place-items-center rounded-lg text-slate-500 transition-colors hover:bg-slate-100 lg:hidden"
              aria-label="Open menu"
            >
              <FaIcon icon="fa-solid fa-bars" size={18} />
            </button>
            <div className="flex items-center cursor-pointer lg:hidden" onClick={() => navigate('/dashboard')}>
